...
from ..chatinterface import MCmd, MInterface
...
class MLobby:
  ...
  def handle_in(self, sender_id, min_p=DEFAULT_MIN_PLAYERS):
    ...
    self.lobby_cast(get_resp("IN",**locals()))
    return True
    
  def handle_out(self, sender_id):
    if sender_id in self.in_list:
      del self.in_list[sender_id]
      self.lobby_cast(get_resp("OUT",**locals()))
    else:
      self.lobby_cast(get_resp("OUT_NOT_IN",**locals()))
    return True

  def handle_start(self, minutes=DEFAULT_TIMER_MINUTES, min_p=DEFAULT_MIN_PLAYERS):
    self.start_msg_id = self.lobby_cast(get_resp("START_TIMER",**locals()))
  ...
  def handle_status(self, g_id=None):
    if len(self.game_ids) == 0:
      self.lobby_cast(get_resp("STATUS_NO_GAMES"))
      return True
    if not g_id:
      g_id = self.game_ids[0]
    game = self.ctrl.games[g_id]
    self.lobby_cast(game.mstate.main_status())
  ...
  def try_start_game(self):
    ...
    ack_in_list = [(p_id, self.start_min_players) for \
      p_id in self.minter.getAcks(self.lobby_id, self.start_msg_id)]
    ...
    while len(full_in_list) > 0:
      ...
      if len(full_in_list) >= full_in_list[0][1]:
        for user_id,_ in full_in_list:
          users[user_id] = self.minter.getName(self.lobby_id, user_id)
        break
      else:
